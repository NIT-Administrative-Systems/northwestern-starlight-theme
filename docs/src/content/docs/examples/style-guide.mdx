---
title: Style Guide
description: A comprehensive reference of all Starlight components styled by the Northwestern theme.
sidebar:
    order: 1
    badge:
        text: Demo
...
Content below a long heading. Verify the anchor link icon stays aligned and Poppins font wraps without clipping.
...
These components are stacked with no separators to demonstrate margin collapse and spacing between different styled blocks.
