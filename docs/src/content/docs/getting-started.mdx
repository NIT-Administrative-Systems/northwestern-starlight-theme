...
<Aside type="note">
pnpm users must install `canvaskit-wasm` directly to enable OG images (a [known pnpm limitation](https://github.com/delucis/astro-og-canvas/tree/latest/packages/astro-og-canvas#pnpm) with native WASM packages). Without it, the build skips OG generation and logs a warning.
